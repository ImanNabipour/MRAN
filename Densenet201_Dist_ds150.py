...
# checkpoint_dir = "D:/Iman_Nabipour/ToTuneCNNs2/Base-dir/cpt/20230129-172032" # checkpoint for run on colab
...
class ResNet(nn.Module):
    def __init__(self, in_channels, outputs=1):
        super().__init__()
        self.layer0 = nn.Sequential(
            nn.Conv3d(in_channels, 10, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm3d(10),
            nn.ReLU()
        )

        self.layer1 = nn.Sequential(
            resblock(10, 10, downsample=False),
            resblock(10, 10, downsample=False),
        ...
        self.layer2 = nn.Sequential(
            ...
            resblock(20, 20, downsample=False),
            # torch.nn.Dropout3d(p=0.1)
        )

        self.layer3 = nn.Sequential(
            resblock(20, 40, downsample=True),
            resblock(40, 40, downsample=False),
            # torch.nn.Dropout3d(p=0.1)
        )

        self.fc = nn.Linear(30*30*30*40, outputs)
...
#%% Densenet
"""
Created on Sun Feb  5 10:15:53 2023
Reference link:
...
model = DenseNet(n_input_channels=1 ,growth_rate=32, block_config=(6, 12, 48, 32), num_init_features=64, num_classes=1,)
out = model(torch.randn([1,1,120,120,120]))
print(out.shape)
#%% SimpleNet
...
#%%
    ...
    # elif m == 3:
    print('you chose Densenet201 model')
    model = DenseNet(n_input_channels=1 ,growth_rate=32, block_config=(6, 12, 48, 32), num_init_features=64, num_classes=1,)
